module PrivyModule
    ...
    def self.status_of_registration(token)
        url = "#{ENV["PRIVY_API_URL"]}/registration/status"
        request = Typhoeus::Request.new(
            url,
            method: :post,
            userpwd: ENV["PRIVY_PASSWORD"],
    ...
    def self.upload_document(doc_title, signerPrivyID, ownerPrivyID, file)
        url = "#{ENV["PRIVY_API_URL"]}/document/upload"
        request = Typhoeus::Request.new(
            url,
            method: :post,
            userpwd: ENV["PRIVY_PASSWORD"],
    ...
    def self.check_document_status(doc_token)
        url = "#{ENV["PRIVY_API_URL"]}/document/status/{doc_token}".gsub('{doc_token}', doc_token)
        request = Typhoeus::Request.new(
            url,
            method: :get,
            userpwd: ENV["PRIVY_PASSWORD"],
